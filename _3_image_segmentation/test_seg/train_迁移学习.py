...
from data_切割道检测 import transform1, transform2, transform_val
...
class data_seg(Dataset):
    def __init__(self, data_path, transform1=None, transform2=None):
        ...
        self.ImagesPath = [os.path.join(data_path, f) for f in os.listdir(data_path) if f.endswith('.jpg')]
        self.LabelsPath = [os.path.join(data_path, f) for f in os.listdir(data_path) if f.endswith('.png')]

        self.Images = []
        self.Labels = []
        for f1,f2 in zip(self.ImagesPath,self.LabelsPath):
            file_name1, extension1 = os.path.splitext(os.path.basename(f1))
            file_name2, extension2 = os.path.splitext(os.path.basename(f2))

            assert file_name1 == file_name2,"文件不相同！"

            image = Image.open(f1).convert("RGB")
            label = Image.open(f2).convert("L")
...
def train(opt):
    ...
    net = deeplabv3(True)  # deeplabv3() UNet()
    net.to(device)
    ...
    for epoch in range(1, opt.epoch):
        ...
        for images, labels in dataloader_train:
            ...
            for i in range(1000):
                ...
                print(f"epoch:{epoch}, loss_train:{round(loss_train, 6)}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain', default='./run/train/best_qgd1.pth', help='指定权重文件，未指定则使用官方权重！')  # 修改
    parser.add_argument('--out_path', default='./run/train', type=str)  # 修改
    parser.add_argument('--weights', default='best_qgd1.pth', help='指定权重文件，未指定则使用官方权重！')

    parser.add_argument('--resume', default=False, type=bool, help='True表示从--weights参数指定的epoch开始训练,False从0开始')
    parser.add_argument('--data_path_train', default='D:/desktop/qgd1/train')
    parser.add_argument('--data_path_val', default='D:/desktop/qgd1/val')
